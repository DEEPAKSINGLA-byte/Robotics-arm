...
import sys
import time
from experiment_io import file_hash as sha, save_json as save
...
def select(args):
    ...
    from run_experiment2 import StableMaskGenerationPipeline, generate_masks, filter_proposals
...
def main():
    ...
    if args.stage == 'select':
        select(args)
        return
    from task3_grasp_proposals import GraspConfig, save_grasps, validate_transform, recovery_instructions
    ...
    subprocess.run([sys.executable, '-B', str(Path(__file__).resolve()), '--stage', 'select',
                    '--config', str(args.config),
                    '--image', str(args.image), '--sentence', args.sentence,
                    '--output', str(args.output / 'selection')], check=True, timeout=180)
    ...
    if read(args.output / 'selection/selection.json')['selected_index'] is None:
        ...
        return
    t = time.perf_counter()
    subprocess.run([str(depth_python), '-B', str(REPO / 'run_depth_scale.py'),
